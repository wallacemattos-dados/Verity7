...
from db.repository import salvar_log_auditoria, listar_logs_auditoria, obter_log_auditoria
...
async def listar_capturas(x_user_id: str = Header(None)):
    """
    Retorna a lista de todas as capturas do usuário autenticado.
    O ID do usuário é injetado pelo Gateway através do header X-User-Id.
    """
    if not x_user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Não autorizado. Header x-user-id ausente no proxy."
        )
    
    logs = listar_logs_auditoria(x_user_id)
    return logs

@app.get("/api/captures/{registro_id}")
async def detalhe_captura(registro_id: str, x_user_id: str = Header(None)):
    """
    Retorna os detalhes de uma captura específica, garantindo isolamento de dados por usuário.
    """
    if not x_user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
    ...
    log = obter_log_auditoria(registro_id, x_user_id)
    if not log:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail="Captura não encontrada ou você não tem permissão para acessá-la."
        )
