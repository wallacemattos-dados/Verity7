...
export default function NovaCapturaPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const nextIdx = idx < STATUS_MESSAGES.length - 2
        ? idx + 1
  ...
  async function handleStart(e: React.FormEvent) {
    ...
    try {
      const res = await solicitarCaptura(token!, url, titulo, user!.id);
      ...
      if (!mountedRef.current) return;
      setError(err instanceof Error ? err.message : 'Erro na captura. Verifique se o backend está rodando.');
      setStep(1);
    }
  }
    ...
    <div className="app-layout animate-fade-in">
      ...
      <main style={{ padding: '2rem', backgroundColor: 'var(--color-bg)', overflowY: 'auto' }}>
          ...
          {STEPS.map((label, i) => {
            ...
            return (
              <div key={label} style={{ display: 'flex', alignItems: 'center' }}>
                ...
                {i < STEPS.length - 1 && (
                  <div style={{
                    width: '80px', height: '2px', margin: '0 0.75rem', marginBottom: '1.5rem',
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          {step === 1 && (
            <StepConfig
              url={url} titulo={titulo} error={error}
              onUrlChange={setUrl} onTituloChange={setTitulo}
          ...
          {step === 3 && resultado && (
            <StepConcluido
              resultado={resultado} titulo={titulo} url={url}
              onNovaCap={() => { setUrl(''); setTitulo(''); setResultado(null); setError(''); setStep(1); }}
            />
          )}
        </div>
      </main>
...
/* ── Step 1 ──────────────────────────────────────────────── */
function StepConfig({ url, titulo, error, onUrlChange, onTituloChange, onSubmit }: {
  url: string; titulo: string; error: string;
  onUrlChange(v: string): void; onTituloChange(v: string): void;
  onSubmit(e: React.FormEvent): void;
}) {
  return (
    <div>
      <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Iniciar Nova Captura</h2>
      ...
      <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        <div>
          ...
          <input type="url" className="input-field"
            ...
            value={url} onChange={e => onUrlChange(e.target.value)} required />
        </div>
        <div>
          <label style={labelStyle}>Título do Registro</label>
          <input type="text" className="input-field"
            placeholder="Ex: Post difamatório no Instagram"
            value={titulo} onChange={e => onTituloChange(e.target.value)} required />
        </div>

        <div style={{
          display: 'flex', gap: '0.875rem', alignItems: 'flex-start',
          padding: '1rem', background: 'var(--color-warning-bg)',
          borderRadius: 'var(--radius-sm)', fontSize: '0.875rem',
          color: 'var(--color-warning-text)',
        }}>
          <InfoIcon />
          <p>
            O Veridit captura a tela em ambiente isolado e aplica imediatamente um{' '}
            <strong>hash SHA-256</strong>. A operação pode levar até 30 segundos.
          </p>
        </div>
        ...
        {error && (
          <p style={{
            ...
            color: 'var(--color-danger)', borderRadius: 'var(--radius-sm)', fontSize: '0.875rem',
          }}>
            {error}
          </p>
        )}
...
/* ── Step 2 ──────────────────────────────────────────────── */
function StepProcessing({ statusMsg, progress }: { statusMsg: string; progress: number }) {
  return (
    <div style={{ textAlign: 'center', padding: '2rem 0' }}>
      ...
      <h2 style={{ fontSize: '1.375rem', marginBottom: '1rem' }}>{statusMsg}</h2>
...
function StepConcluido({ resultado, titulo, url, onNovaCap }: {
  resultado: CapturaResponse; titulo: string; url: string; onNovaCap(): void;
}) {
  function copyHash() {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: '1.875rem', marginBottom: '0.5rem' }}>Captura Concluída!</h2>
      ...
      {resultado.screenshot_base64 && (
        <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: '1.5rem', textAlign: 'left' }}>
          <p style={{
      ...
      <div className="card" style={{ textAlign: 'left', marginBottom: '2rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', fontSize: '0.875rem' }}>
          <ResultField label="ID do Registro" value={resultado.id_registro} mono />
          ...
          <ResultField label="Tamanho" value={`${(resultado.tamanho_bytes / 1024).toFixed(1)} KB`} mono />
        </div>
        ...
        <div style={{
          ...
          <p style={{
            ...
            wordBreak: 'break-all', color: 'var(--color-primary)', fontWeight: 600,
          }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        <button onClick={onNovaCap} className="btn btn-outline">Nova Captura</button>
        <button
...
/* ── Helpers ─────────────────────────────────────────────── */
function ResultField({ label, value, mono, small }: { label: string; value: string; mono?: boolean; small?: boolean }) {
  return (
    <div>
      <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem', fontWeight: 600 }}>
        {label}
      </p>
      <p style={{
        fontFamily: mono ? 'var(--font-jetbrains)' : undefined,
        fontSize: small ? '0.75rem' : '0.9375rem',
        fontWeight: mono ? 600 : 700, wordBreak: 'break-all',
      }}>
        {value}
      </p>
    </div>
